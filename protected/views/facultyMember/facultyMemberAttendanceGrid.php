...
input {
    ...
    height: 30px !important;
		...
		<?php 
		//echo "SELECT distinct(session_id) FROM class_routine where (faculty_member_id = '".$model->member_pk."' or additional_faculty_member_id='".$model->member_pk."') order by session_id desc";
		$connection=Yii::app()->db;
        $command=$connection->createCommand("SELECT distinct(session_id) FROM class_routine where (faculty_member_id = '".$model->member_pk."' or additional_faculty_member_id='".$model->member_pk."') order by session_id desc");
		
        $all_session = $command->queryAll();
		...
		//$nbd=new array();
		$ii=1;
		foreach($all_session as $sess):
			if($ii==1)
			{
			$nbd[]=array('label'=>$sess['session_id'], 'active'=>true, 'content'=>$this->renderPartial('facultyMemberBySession', array('session_id'=>$sess['session_id'],'model'=>$model),true));
		}
		else
		{
		$nbd[]=array('label'=>$sess['session_id'],  'content'=>
		$this->renderPartial('facultyMemberBySession', array('session_id'=>$sess['session_id'],'model'=>$model),true)
