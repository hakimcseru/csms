    	...
    	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="routine_tbl">
		  ...
		  <?php 
		   ...
		   $connection=Yii::app()->db;
			...
			$connection=Yii::app()->db;
			$command=$connection->createCommand("SELECT distinct(batch_group_id) FROM class_routine where $where and weekday='".$cg['weekday']."' order by batch_section_id");
			$groupA = $command->queryAll();
		   ?>
            <td class="no_padding" valign="top">
            	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="routine_tbl" valign="top">
                  <tr>
				  <?php  if($i==1) {?>
                    <td><strong>দল</strong></td>
					<?php  }?>
				  ...
				  $bss=BatchGroup::model()->findByPk($gp['batch_group_id']);
				  
				  ?>
                  <tr>
				  <?php if($i==1) {?>
                    <td class="ht">
                    	<div><?php echo $bss->group_name;?></div>
               
                    </td>
					<?php  }?>
                    <?php foreach($cperiod as $per):
					$srr=ClassRoutine::model()->find($where." and batch_group_id=".$gp['batch_group_id']." and weekday='".$cg['weekday']."' and class_period_id=".$per['class_period_id']);
