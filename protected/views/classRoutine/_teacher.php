...
<?php foreach($fmember as $fmem):
			...
			$fm=FacultyMember::model()->findByPk($fmem['faculty_member_id']);
			$connection=Yii::app()->db;
			$command=$connection->createCommand("SELECT distinct(class_period_id) FROM class_routine where $where and faculty_member_id = '".$fmem['faculty_member_id']."' order by id");
			$cperiod = $command->queryAll();
			
			
			$connection=Yii::app()->db;
			$command=$connection->createCommand("SELECT distinct(batch_section_id) FROM class_routine where $where and faculty_member_id = '".$fmem['faculty_member_id']."' order by batch_section_id");
			$section = $command->queryAll();
			
			
			$connection=Yii::app()->db;
			$command=$connection->createCommand("SELECT distinct(weekday) FROM class_routine where $where and faculty_member_id = '".$fmem['faculty_member_id']."' order by batch_section_id");
    ...
    <td>
    	<table width="100%" border="0" cellspacing="0" cellpadding="0" align="center" class="routine_tbl">
		  ...
		  <tr>
			...
			$srr2=ClassRoutine::model()->findAll($where." and faculty_member_id = '".$fmem['faculty_member_id']."' and weekday='".$wd['weekday']."' order by class_period_id");
			foreach($srr2 as $srr):
