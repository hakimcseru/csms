...
class StudentAccountClearanceController extends Controller
{
	/**
	...
	/*public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('create','update','qcreate','DisCoor','Group','Generate'),
				'users'=>array('@'),
			),
  ...
  public function actionGroup() {
	...
	public function actionCreate()
	{
		...
		$this->renderpartial('print',array(
			'stids'=>$_POST['stlist'],'subjects'=>$_POST['subjects'],
		));
		
		
		}
		elseif(isset($_POST['stlist']))
		{
		
		$this->renderpartial('idview'.$_POST['type'],array(
			'stids'=>$_POST['stlist'],'type'=>$_POST['type'],'subjects'=>$_POST['subjects'],
		));
     ...
     public function actionList()
	{
		...
		if(isset($_POST['Student']))
		{
			...
			if($where2) $where2 .=" and course_subject_ref_course_pk =".$course; else $where2.=" course_subject_ref_course_pk =".$course;
			}
			...
			if($where2) $where2 .=" and course_subject_department_id =".$department; else $where2.=" course_subject_department_id =".$department;
			}
			...
			if($semester) {
			if($where) $where .=" and semester =".$semester; else $where.=" semester =".$semester;
			if($where2) $where2 .=" and course_subject_semester_no =".$semester; else $where2.=" course_subject_semester_no =".$semester;
			...
			$subjects=CourseSubject::model()->findAll($where2);
			
			$att_set=AttendanceReleaseSetting::model()->find("session='".$session."' and course_id='".$course."'");
		...
		$this->render('list',array(
			'model'=>$studentEnInfo,'card_type'=>$card_type,'session_id'=>$session,'subjects'=>$subjects,'att_set'=>$att_set
		));
		exit();
		/*
        ...
        public function actionGenerate()
	{
		...
		// $this->performAjaxValidation($model);
			...
			$this->layout='//layouts/'.$card_type;
			$this->renderPartial('_p'.$card_type,array(
			'model'=>$dp,
		));
		exit();
		/*
