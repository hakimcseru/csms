...
'users'=>array('*'),
	...
	{
		$model=new StudentDues;

		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);

		if(isset($_POST['StudentDues']))
		{
			$model->attributes=$_POST['StudentDues'];
			if($model->save())
	...
	{
		$dataProvider=new CActiveDataProvider('StudentDues');
	...
	{
		$model=new StudentDues('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['StudentDues']))
			$model->attributes=$_GET['StudentDues'];
...
array('name'=>'session_id','value'=>'Bndate::BanglaNumDate($data->session_id)'),
		array('name'=>'course_id','value'=>'$data->course->course_name'),
...
array('header'=>'Batch Group','value'=>'$data->studentsenrollment?$data->studentsenrollment->batchgroup->group_name:""'),
	...
	{
		$model=StudentDues::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='student-dues-form')
		{
